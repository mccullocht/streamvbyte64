...
#[inline]
fn encode_to_sink<G: UnsafeGroup, H: EncodeSink<G>>(
    values: &[G::Elem],
    tags: &mut [u8],
    data: &mut [u8],
    sink: &mut H,
    ...
    assert!(num_groups * G::TAG_LEN[3] as usize * 4 <= data.len());
...
pub(crate) fn encode<G: UnsafeGroup>(
    values: &[G::Elem],
    tags: &mut [u8],
    encoded: &mut [u8],
) -> usize {
...
#[inline]
pub(crate) fn encode_deltas<G: UnsafeGroup>(
    initial: G::Elem,
    values: &[G::Elem],
    tags: &mut [u8],
    encoded: &mut [u8],
...
#[inline]
fn decode_to_sink<G: UnsafeGroup, S: DecodeSink>(tags: &[u8], data: &[u8], sink: &mut S) -> usize {
    let mut read = 0usize;
    let mut tag_index = 0;
    for tag_chunk in tags.chunks_exact(8) {
        ...
        // TODO: add a slop constant to UnsafeGroup; would be smaller for scalar implementations (size_of<Elem>() - 1)
        let max_read = G::data_len8(tag8) + (G::TAG_LEN[3] - G::TAG_LEN[0]) as usize * 4;
    ...
    for tag in &tags[tag_index..] {
        if read + G::TAG_LEN[3] as usize * 4 > data.len() {
            break;
        }
    ...
    if !remainder.is_empty() {
        ...
        for tag in remainder {
            ...
            assert!(bufr < G::TAG_LEN[3] as usize * 4);
...
#[inline]
pub(crate) fn decode<G: UnsafeGroup>(tags: &[u8], encoded: &[u8], values: &mut [G::Elem]) -> usize {
    assert_eq!(values.len() % 4, 0);
...
#[inline]
pub(crate) fn decode_deltas<G: UnsafeGroup>(
    initial: G::Elem,
    tags: &[u8],
    encoded: &[u8],
    values: &mut [G::Elem],
...
#[inline]
pub(crate) fn data_len<G: UnsafeGroup>(tags: &[u8]) -> usize {
    let mut len = 0usize;
...
struct SkipDeltasSink<G: UnsafeGroup>(G::Elem);
...
#[inline]
pub(crate) fn skip_deltas<G: UnsafeGroup>(tags: &[u8], data: &[u8]) -> (usize, G::Elem)
where
    <G as UnsafeGroup>::Elem: WrappingAdd,
{
