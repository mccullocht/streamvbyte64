...
/// Group64 encode and decodes a compact, byte-aligned format where groups of 4 64-bit integers are encoded together.
...
pub trait Group64: Sized + Copy + Clone {
    ...
    fn encode_to_writer<W: Write>(
        ...
        let mut buf = [0u8; 256];
        let mut written = 0usize;
        for (chunk_values, chunk_tags) in values.chunks(64).zip(tags.chunks_mut(8)) {
            let nbuf = self.encode(chunk_values, chunk_tags, &mut buf);
            data.write(&buf[..nbuf])?;
            written += nbuf;
        }
